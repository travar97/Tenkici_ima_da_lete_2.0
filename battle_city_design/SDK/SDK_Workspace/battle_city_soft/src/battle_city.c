...
static void process_ai( tank_t * tank, unsigned int * ai_dir )
{
	...
	if( *ai_dir == DIR_DOWN ) {
		while( tank->y / 8 + i < MAP_Y + MAP_H ) {
			if(mape[ ( tank->y / 8 + i ) * MAP_WIDTH + tank->x / 8 ].ptr == IMG_8x8_IRON ) {
				break;
			}
			else if(mape[ ( tank->y / 8 + i ) * MAP_WIDTH + tank->x / 8 ].ptr == IMG_8x8_NULL){
				fire = b_false;
				break;
			}
			else if(mape[ ( tank->y / 8 + i ) * MAP_WIDTH + tank->x / 8 ].ptr == IMG_8x8_BLANK){
				fire = b_false;
				break;
			}else if(mape[ ( tank->y / 8 + i ) * MAP_WIDTH + tank->x / 8 ].ptr == IMG_8x8_BRICK){
				fire = b_true;
			}
			 else if( hitbox( tank->x, tank->y + i * 8, tank1.x, tank1.y, 16, 16 ) == b_true ) {
				fire = b_true;
