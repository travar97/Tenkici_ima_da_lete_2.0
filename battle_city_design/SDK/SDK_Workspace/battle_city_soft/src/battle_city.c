...
base_t base = {
		( MAP_X + MAP_W / 2 +2) * 8 - 24 ,	//311
		( MAP_Y+2 + MAP_H - 5 ) * 8 + 8 ,	//463

		IMG_16x16_BASE_ALIVE,  				// type
...
static void obj_spawn(tank_t * tenkzi)
{
	Xil_Out32(
...
static void base_spawn(base_t * base)
{
	Xil_Out32(
			...
			(u32 )0x8F000000 | 0 | (u32 )base->type);
	Xil_Out32(
			XPAR_BATTLE_CITY_PERIPH_0_BASEADDR + 4 * ( REGS_BASE_ADDRESS + base->reg_h ),
			(base->y << 16) | base->x);
}
...
static void direction_veto()
{
	if (tank_ai.dir==tank_ai2.dir){
		if(tank_ai3.dir==tank_ai.dir){
			tank_ai3.dir=change_dir(tank_ai3.dir);
						}
		if(tank_ai4.dir==tank_ai.dir){
			tank_ai4.dir=change_dir(tank_ai4.dir);
						}
	}else if(tank_ai.dir==tank_ai3.dir){
		if(tank_ai4.dir==tank_ai.dir){
			tank_ai4.dir=change_dir(tank_ai4.dir);
				}
	}else if(tank_ai2.dir==tank_ai3.dir){
		if(tank_ai4.dir==tank_ai2.dir){
			tank_ai4.dir=change_dir(tank_ai4.dir);
		}
...
static void process_bullet( map_entry_t * map, bullet_t * blt )
{
    ...
    if( blt->explosion ) {
        ...
        if( game_time > blt->time + BULLET_STEP_DURATION ) {
			...
			br = &map[ ( ( y + 11 ) / 8 ) * MAP_WIDTH + ( ( x + 11 ) / 8 ) ];
...
static void process_ai3( tank_t * tank, unsigned int * ai_dir3 )
{
	...
	/*
	if((rand_lfsr113( ) % 4) ==2){
		tank_move( mape, &tank_ai3, DIR_DOWN);
	}
	if((rand_lfsr113( ) % 4) ==3){
		tank_move( mape, &tank_ai3, DIR_UP);
	}
	if((rand_lfsr113( ) % 4) ==1){
			tank_move( mape, &tank_ai3, DIR_LEFT);
		}
	if((rand_lfsr113( ) % 4) ==0){
			tank_move( mape, &tank_ai3, DIR_RIGHT);
	}
	*/
...
void konverzija(map_entry_t *mapa, uint8_t *map)
{
	...
	j=0;
	while(j<25) {
		for(k = 488+j*step; k <= 514+j*step; k++)
		{
	...
	i=0;							// Right
	while(i<27) {
		for(j = 354+i*step; j <= 399+i*step; j++)
		{
			mapa[j].z = 0;
			mapa[j].rot = 0;
			mapa[j].ptr = 0x03A0;
...
void battle_city( void )
{
    ...
    while(1) {
			...
			if( game_result == RES_VICTORY ) {
				...
				draw_string( 18, 1, game_loss );
				if(base.destroyed==b_true){
				base.type=IMG_16x16_BASE_DEAD;
				base_spawn( &base);
				}
